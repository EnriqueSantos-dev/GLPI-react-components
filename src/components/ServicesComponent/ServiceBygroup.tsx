...
const ServiceBySubgroup = ( ) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const [servicesResponse, subgroupResponse] = await Promise.all([
		getAllServices(router.query.subgroupId as string),
		getSubGroup(router.query.subgroupId as string)
	  ]);
...
export default ServiceBySubgroup;
